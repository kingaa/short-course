...
#'     $$\dlta{\tilde N}_{SI}=\dist{Poisson}{\tilde \mu_{SI}(t)\,\tilde S(t)\,\delta},$$ where $\dist{Poisson}{\mu}$ is a Poisson random variable with mean $\mu$ and $$\tilde\mu_{SI}(t)=\beta\,\frac{\tilde I(t)}{N}.$$
#'     1. Binomial increments with linear probability:
#'     $$\dlta{\tilde N}_{SI}=\dist{Binomial}{\tilde{S}(t),\tilde\mu_{SI}(t)\,\delta},$$ where $\dist{Binomial}{n,p}$ is a binomial random variable with mean $n\,p$ and variance $n\,p\,(1-p)$.
#'     1. $\dlta{\tilde{N}}_{SI}=\dist{Binomial}{\tilde{S}(t),1-e^{-\tilde{\mu}_{SI}(t)\,\delta}}$.  
...
#' ## [**R** codes for this document](http://raw.githubusercontent.com/kingaa/short-course/gh-pages/stochsim/stochsim.R)
#' 
#' --------------------------
#' 
#' ## References
