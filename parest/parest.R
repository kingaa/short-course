...
#' 
## ----sse-betahat---------------------------------------------------------
params4 <- c(Beta=beta.hat,gamma=1,N=50000,S_0=10000,I_0=10)
coef(niameyA) <- params4
...
f2 <- function (par) {
  params <- c(Beta=NA,gamma=1,N=50000,S_0=NA,I_0=NA)
...
coef(niameyA) <- c(S_0=20000,I_0=1,gamma=1,mle1)
model.pred <- trajectory(niameyA)["I",,]

library(plyr)
raply(2000,rpois(n=length(model.pred),lambda=coef(niameyA,"p")*model.pred)) -> simdat
