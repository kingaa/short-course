...
```{r nine-curves,echo=FALSE,warning=FALSE,purl=FALSE}
...
```{r final-size,echo=F,purl=FALSE}
...
```{r bbs-flu-lm,echo=T,results='markup',purl=FALSE}
fit1 <- lm(log(B)~day,data=subset(bsflu,day<=4))
summary(fit1)
coef(fit1)
...
```{r inv-R0-est,echo=F,purl=FALSE}
...
```
```{r niamey-plot,purl=FALSE,eval=FALSE}
...
```{r niamey-plot,warning=FALSE,purl=FALSE,echo=FALSE}
```
...
f6 <- function (pars) {
  params <- c(S_0=20000,I_0=1,gamma=1,b=NA,p=NA)
  params[c("b","p")] <- pars
  poisson.loglik(params)
}
...
coef(niameyA) <- c(S_0=20000,I_0=1,gamma=1,mle3)
model.pred <- trajectory(niameyA)["I",,]

library(plyr)
raply(2000,rnbinom(n=length(model.pred),
                   mu=coef(niameyA,"p")*model.pred,
                   size=1/coef(niameyA,"theta"))) -> simdat
aaply(simdat,2,quantile,probs=c(0.025,0.5,0.975)) -> quantiles
